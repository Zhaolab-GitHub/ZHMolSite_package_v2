...
class RNA_PNAConv(nn.Module):
    def __init__(self, RNA_deg, hidden_channels, edge_channels,
                 pre_layers=2, post_layers=2,
                 aggregators=['sum', 'mean', 'min', 'max', 'std'],
                 scalers=['identity', 'amplification', 'attenuation'],
                 num_towers=4,
                 dropout=0.1):
        super(RNA_PNAConv, self).__init__()
    ...
    def forward(self, x, RNA_edge_index, RNA_edge_attr):
        x_in = x
        x = x_in + F.relu(self.norm(self.conv(x, RNA_edge_index, RNA_edge_attr)))
...
class RNAProteinConv(MessagePassing):
    _alpha: OptTensor
    ...
    def __init__(
        ...
        super(RNAProteinConv, self).__init__(node_dim=0, **kwargs)

        assert residue_channels % heads == 0
        assert nuc_channels % heads == 0

        self.residue_out_channels = residue_channels // heads
        self.nuc_out_channels = nuc_channels // heads
        self.heads = heads
        self.edge_dim = edge_dim
        self._alpha = None

        ## Protein Residue -> RNA Nucleotide
        self.prot_lin_key = nn.Linear(residue_channels, heads * self.nuc_out_channels, bias=False)
        self.prot_lin_query = nn.Linear(nuc_channels, heads * self.nuc_out_channels, bias=False)
        self.prot_lin_value = nn.Linear(residue_channels, heads * self.nuc_out_channels, bias=False)
        if edge_dim is not None:
            self.prot_lin_edge = nn.Linear(edge_dim, heads * self.nuc_out_channels, bias=False)
        else:
            self.prot_lin_edge = self.register_parameter('prot_lin_edge', None)

        ## RNA Nucleotide -> Protein Residue
        self.RNA_lin_key = nn.Linear(nuc_channels, heads * self.residue_out_channels, bias=False)
        self.RNA_lin_query = nn.Linear(residue_channels, heads * self.residue_out_channels, bias=False)
        self.RNA_lin_value = nn.Linear(nuc_channels, heads * self.residue_out_channels, bias=False)
        if edge_dim is not None:
            self.RNA_lin_edge = nn.Linear(edge_dim, heads * self.residue_out_channels, bias=False)
        else:
            self.RNA_lin_edge = self.register_parameter('RNA_lin_edge', None)


        ## Normalization
        self.nuc_in_norm = torch.nn.LayerNorm(nuc_channels)
        self.residue_in_norm = torch.nn.LayerNorm(residue_channels)

        self.nuc_out_norm = torch.nn.LayerNorm(heads * self.nuc_out_channels)
        self.residue_out_norm = torch.nn.LayerNorm(heads * self.residue_out_channels)
        ## MLP
        self.nuc_mlp = MLP([nuc_channels * 2, nuc_channels * 2, nuc_channels], out_norm=True)
    ...
    def reset_parameters(self):
        self.prot_lin_key.reset_parameters()
        self.prot_lin_query.reset_parameters()
        self.prot_lin_value.reset_parameters()

        self.RNA_lin_key.reset_parameters()
        self.RNA_lin_query.reset_parameters()
        self.RNA_lin_value.reset_parameters()
        if self.edge_dim:
            self.prot_lin_edge.reset_parameters()
        if self.edge_dim:
            self.RNA_lin_edge.reset_parameters()

        ### normalization
        self.nuc_in_norm.reset_parameters()
        self.residue_in_norm.reset_parameters()
        self.nuc_out_norm.reset_parameters()
        self.residue_out_norm.reset_parameters()

        # MLP update
        self.nuc_mlp.reset_parameters()
        self.residue_mlp.reset_parameters()

    def forward(self, nuc_x, residue_x, edge_index: Adj):

        residue_hx = self.residue_in_norm(residue_x)  # normalization
        nuc_hx = self.nuc_in_norm(nuc_x)  # normalization
        # print(residue_hx.shape)
        # print(residue_hx)
        # print(nuc_hx.shape)
        # print(nuc_hx)

        # Protein Residue -> RNA Nucleotide
        H, aC = self.heads, self.nuc_out_channels
        query_res = self.prot_lin_query(nuc_hx).view(-1, H, aC)
        key_res = self.prot_lin_key(residue_hx).view(-1, H, aC)
        value_res = self.prot_lin_value(residue_hx).view(-1, H, aC)

        # RNA Nucleotide -> Protein Residue
        H, rC = self.heads, self.residue_out_channels
        query_rna = self.RNA_lin_query(residue_hx).view(-1, H, rC)
        key_rna = self.RNA_lin_key(nuc_hx).view(-1, H, rC)
        value_rna = self.RNA_lin_value(nuc_hx).view(-1, H, rC)

        # print(query_res.shape)
        # print(query_res)
        # print(key_res.shape)
        # print(key_res)
        # print(value_res.shape)
        # print(value_res)
        #
        # print(query_rna.shape)
        # print(query_rna)
        # print(key_rna.shape)
        # print(key_rna)
        # print(value_rna.shape)
        # print(value_rna)
        # print(edge_index.shape)
        # print(edge_index)

        # Bidirectional propagation
        nuc_out = self.propagate(edge_index, query=query_res, key=key_res, value=value_res,
                                 edge_attr=None, size=None)
        alpha_nuc = self._alpha
        # print(alpha_nuc.shape)
        # print(alpha_nuc)
        self._alpha = None

        res_out = self.propagate(edge_index.flip(0), query=query_rna, key=key_rna, value=value_rna,
                                 edge_attr=None, size=None)
        alpha_res = self._alpha
        # print(alpha_res.shape)
        # print(alpha_res)
        # Process outputs
        nuc_out = nuc_out.view(-1, H * aC)
        nuc_out = self.nuc_out_norm(nuc_out)
        nuc_out = torch.cat([nuc_out, nuc_x], dim=-1)
        nuc_out = self.nuc_mlp(nuc_out)

        res_out = res_out.view(-1, H * rC)
        res_out = self.residue_out_norm(res_out)
        res_out = torch.cat([res_out, residue_x], dim=-1)
        res_out = self.residue_mlp(res_out)

        return nuc_out, res_out, (edge_index, alpha_nuc, alpha_res)
    ...
    def message(self, query_i: Tensor, key_j: Tensor, value_j: Tensor,
        ...
        alpha = alpha / self.t  # temperature scaling
